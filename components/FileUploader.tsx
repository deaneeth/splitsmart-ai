...
export const FileUploader: React.FC<FileUploaderProps> = ({ onFileSelect, isAnalyzing }) => {
  ...
  return (
    <div
      className={`relative flex flex-col items-center justify-center w-full h-80 rounded-2xl transition-all duration-300 overflow-hidden ${
        isAnalyzing
    ...
    >
      {isAnalyzing ? (
        <div className="flex flex-col items-center z-10">
          ...
          <p className="text-xl font-semibold text-gray-900 dark:text-white">Analyzing Receipt</p>
          <p className="text-sm text-indigo-600 dark:text-indigo-400 mt-2 font-medium animate-pulse">Gemini is reading the items...</p>
        </div>
      ) : (
        <label className="flex flex-col items-center justify-center w-full h-full cursor-pointer group">
          <div className="flex flex-col items-center justify-center p-6 text-center transition-transform group-hover:scale-105 duration-300">
            <div className="p-4 bg-white dark:bg-gray-700 rounded-full mb-4 shadow-sm group-hover:shadow-md group-hover:bg-indigo-600 group-hover:text-white dark:group-hover:bg-indigo-500 transition-all duration-300">
              <Upload className="w-8 h-8 text-indigo-600 dark:text-indigo-400 group-hover:text-white transition-colors" />
            </div>
